...
/**
 * REST controller for managing {@link com.isep.desofs.domain.Payment}.
 */
@RestController
...
public class PaymentResource {
     ...
     */
    @GetMapping("")
    public List<PaymentDTO> getAllPayments(@RequestParam(name = "filter", required = false) String filter) {
        if ("cart-is-null".equals(filter)) {
            log.debug("REST request to get all Payments where cart is null");
            return paymentService.findAllWhereCartIsNull();
        }
