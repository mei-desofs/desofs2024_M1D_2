...
import { EntityArrayResponseType, RoleMySuffixService } from '../service/role-my-suffix.service';
...
export class RoleMySuffixComponent implements OnInit {
  ...
  trackId = (_index: number, item: IRoleMySuffix): number => this.roleService.getRoleMySuffixIdentifier(item);
  ...
  protected refineData(data: IRoleMySuffix[]): IRoleMySuffix[] {
    const { predicate, order } = this.sortState();
    return predicate && order ? data.sort(this.sortService.startSort({ predicate, order })) : data;
  }

  protected fillComponentAttributesFromResponseBody(data: IRoleMySuffix[] | null): IRoleMySuffix[] {
    return data ?? [];
  }
