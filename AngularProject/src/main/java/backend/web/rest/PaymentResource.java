...
/**
 * REST controller for managing {@link backend.domain.Payment}.
 */
@RestController
...
public class PaymentResource {
     ...
     */
    @PostMapping("")
    public Mono<ResponseEntity<PaymentDTO>> createPayment(@RequestBody PaymentDTO paymentDTO) throws URISyntaxException {
    ...
    public Mono<List<PaymentDTO>> getAllPayments(@RequestParam(name = "filter", required = false) String filter) {
        if ("cart-is-null".equals(filter)) {
            log.debug("REST request to get all Payments where cart is null");
            return paymentService.findAllWhereCartIsNull().collectList();
        }
        log.debug("REST request to get all Payments");
        return paymentService.findAll().collectList();
    }

    /**
     * {@code GET  /payments} : get all the payments as a stream.
     * @return the {@link Flux} of payments.
     */
    @GetMapping(value = "", produces = MediaType.APPLICATION_NDJSON_VALUE)
    public Flux<PaymentDTO> getAllPaymentsAsStream() {
        log.debug("REST request to get all Payments as a stream");
